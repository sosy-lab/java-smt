...
public abstract class AbstractProofNode implements ProofNode {
  ...
  @Override
  public ImmutableSet<ProofNode> getChildren() {
    return this.children;
  }

  protected void addChild(ProofNode pChild) {
    Set<ProofNode> tempChildren = new LinkedHashSet<>(this.children);
  ...
  protected String proofAsString(int pIndentLevel) {
    ...
    if (!isLeaf()) {
      for (ProofNode child : getChildren()) {
        ...
        sb.append(((AbstractProofNode) child).proofAsString(pIndentLevel + 1));
      }
    }
