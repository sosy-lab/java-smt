...
public abstract class SolverBasedTest0 {
  ...
  protected void requireParser() {
    assume()
        ...
        .that(solverToUse())
        .isNoneOf(Solvers.CVC4, Solvers.BOOLECTOR, Solvers.YICES2, Solvers.LEANSMT);
  ...
  protected void requireUnsatCore() {
    assume()
        ...
        .that(solverToUse())
        .isNoneOf(Solvers.BOOLECTOR, Solvers.LEANSMT);
  }
  ...
  protected void requireUnsatCoreOverAssumptions() {
    assume()
        .withMessage("Solver %s does not support unsat core generation", solverToUse())
        .that(solverToUse())
        .isNoneOf(Solvers.PRINCESS, Solvers.LEANSMT);
  }
