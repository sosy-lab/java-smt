...
class HardIntegerFormulaGenerator {
  ...
  BooleanFormula generate(int n) {
    ...
    clauses.add(ifmgr.equal(
        ifmgr.makeVariable(COUNTER_PREFIX + 0), ifmgr.makeNumber(0))
    );
    int lastIdx = 0;
    int expected = 0;
    for (int i=1; i < 2 * n; i += 2) {
      BooleanFormula selector = bfmgr.makeVariable(CHOICE_PREFIX + i);
      clauses.add(bfmgr.or(
          mkConstraint(i, 3, selector),
          mkConstraint(i, 2, bfmgr.not(selector))
      ));
      clauses.add(bfmgr.or(
          mkConstraint(i + 1, 3, bfmgr.not(selector)),
          mkConstraint(i + 1, 2, selector)
      ));
      lastIdx = i + 1;
      expected += 5;
    }
    clauses.add(
        ifmgr.greaterThan(
            ifmgr.makeVariable(COUNTER_PREFIX + lastIdx),
            ifmgr.makeNumber(expected)
        )
    );
  ...
  private BooleanFormula mkConstraint(int newIdx, int increment, BooleanFormula selector) {
    return bfmgr.and(
        ...
        ifmgr.equal(
            ...
            ifmgr.add(
                ifmgr.makeVariable(COUNTER_PREFIX + (newIdx - 1)),
                ifmgr.makeNumber(increment)
            )
        )
    );
  }
