...
public class BoolectorBooleanFormulaSubjectTest extends SolverBasedTest0 {
  @Override
  ...
  public void setupFormulas() {
    BitvectorType type = FormulaType.getBitvectorTypeWithSize(2);
    simpleFormula = bvmgr.equal(bvmgr.makeVariable(type, "a"), bvmgr.makeBitvector(2, 1));
    contradiction =
        bmgr.and(
            simpleFormula,
            bvmgr.equal(bvmgr.makeVariable(type, "b"), bvmgr.makeBitvector(2, 2)));
  ...
  /*
   * @Test public void testIsUnsatisfiableYes() throws SolverException, InterruptedException {
  ...
  public void testIsEquisatisfiableoNo() {
    BitvectorType type = FormulaType.getBitvectorTypeWithSize(2);
    BooleanFormula simpleFormula2 =
        bvmgr.equal(bvmgr.makeVariable(type, "d"), bvmgr.makeVariable(type, "2"));
    AssertionError failure =
        expectFailure(
  ...
  private interface ExpectFailureCallback
    ...
    @Override
    default void
        invokeAssertion(SimpleSubjectBuilder<BooleanFormulaSubject, BooleanFormula> pWhenTesting) {
      try {
