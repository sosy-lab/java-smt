 ...
 *
 * This class is not thread-safe, but it could be easily made so by synchronizing the
 * {@link #createNewVar()} method (assuming the BDDFactory is thread-safe).
 */
...
class JavaBDDRegionManager implements RegionManager {
   ...
   * "Size of BDD node cleanup queue"); private final StatTimer cleanupTimer = new
   * StatTimer("Time for BDD node cleanup");
   */
  ...
  private final Map<Reference<? extends JavaBDDRegion>, BDD> referenceMap =
      Maps.newIdentityHashMap();

  @Option(
    secure = true,
    description = "Initial size of the BDD node table in percentage of available Java heap memory (only used if initTableSize is 0).")
    ...
    try {
      Method gcCallback =
          JavaBDDRegionManager.class
              .getDeclaredMethod("gcCallback", Integer.class, BDDFactory.GCStats.class);
      gcCallback.setAccessible(true);
      factory.registerGCCallback(this, gcCallback);

      Method resizeCallback =
          JavaBDDRegionManager.class
              .getDeclaredMethod("resizeCallback", Integer.class, Integer.class);
      resizeCallback.setAccessible(true);
      factory.registerResizeCallback(this, resizeCallback);

      Method reorderCallback =
          JavaBDDRegionManager.class
              .getDeclaredMethod("reorderCallback", Integer.class, BDDFactory.ReorderStats.class);
  ...
  //
  // writingStatisticsTo(out).put("Number of BDD nodes", factory.getNodeNum())
  ...
  private class FormulaToRegionConverter implements AutoCloseable, BooleanFormulaVisitor<BDD> {
    ...
    private BDD
        visitBinary(BooleanFormula pOperand1, BooleanFormula pOperand2, BDDFactory.BDDOp operator) {
