...
class SylvanBDDRegionManager implements RegionManager {
  ...
  private final Map<Reference<SylvanBDDRegion>, Long> referenceMap = Maps.newConcurrentMap();
  ...
  public SylvanBDDRegionManager(Configuration config, LogManager pLogger)
    ...
    Concurrency.newDaemonThread("BDD cleanup thread", new Runnable() {
      @Override
  ...
  public void printStatistics(PrintStream out) {
    synchronized (cleanupTimer) {
      writingStatisticsTo(out).putIf(
          cleanupTimer.getUpdateCount() > 0,
          "Number of BDD freed by GC",
          cleanupTimer.getUpdateCount()).putIfUpdatedAtLeastOnce(cleanupTimer);
    }
  }
