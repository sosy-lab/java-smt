  ...
  SmtInterpolProof createProof(ProvitionalProofNode pProofNode) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (!proofNode.isVisited) {
        ...
        SmtInterpolProofNode pn =
            new SmtInterpolProofNode(proofNode.proofRule, formula);
    ...
    return new SmtInterpolProof(computed.get(pProofNode));
  }
        ...
        if (values[i] instanceof Term) {
          ...
          if (isProves) {
            ...
            if (((String) values[i - 1]).contains("+")) {
              ...
              this.formulas.add(
                  creator.getEnv().getTheory().not((Term) values[i]));
            }
          } else {
            this.formulas.add((Term) values[i]);
          }
