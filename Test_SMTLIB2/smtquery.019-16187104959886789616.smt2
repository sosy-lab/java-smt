(reset)
(set-logic ALL)
(declare-fun __A () Bool)
(declare-fun |id#1@1| () (_ BitVec 32))
(declare-fun |id#2@1| () (_ BitVec 32))
(declare-fun |id#0@1| () (_ BitVec 32))
(declare-fun __string__ (Int) (_ BitVec 32))
(declare-fun Integer_%_ (Int Int) Int)
(declare-fun Integer_*_ (Int Int) Int)
(declare-fun Integer_/_ (Int Int) Int)
(push 1)
(assert (and (and (and (and (and (bvsle (_ bv0 32) |id#0@1|) (= (bvadd |id#2@1| (_ bv1 32)) (_ bv11 32))) (bvsle |id#2@1| (_ bv10 32))) (bvsle (_ bv0 32) |id#2@1|)) (bvsle |id#0@1| (_ bv10 32))) (not (= |id#0@1| |id#2@1|))))
(check-sat)
; (partial-model)
; (get-value ...)
; (get-value ...)
(push 1)
(assert (or (not __A) (not (= |id#0@1| (_ bv9 32)))))
(push 1)
(assert __A)
(check-sat)
(pop 1)
(pop 1)
(push 1)
(assert (or (not __A) (not (= |id#2@1| (_ bv10 32)))))
(push 1)
(assert __A)
(check-sat)
(pop 1)
(pop 1)
(exit)
(reset)
(set-logic ALL)
