...
public class MyServlet extends HttpServlet {
  ...
  public void printSolverInformation(PrintWriter out) throws InterruptedException, SolverException {
      final List<MyServlet.SolverInfo> infos = new ArrayList<>();
      for (Solvers s : Solvers.values()) {
        infos.add(new SolverInfoPrinter().getSolverInformation(s));
      }

      infos.sort(Comparator.comparing(MyServlet.SolverInfo::getName)); // alphabetical ordering

      MyServlet.RowBuilder rowBuilder = new MyServlet.RowBuilder();
      for (MyServlet.SolverInfo info : infos) {
