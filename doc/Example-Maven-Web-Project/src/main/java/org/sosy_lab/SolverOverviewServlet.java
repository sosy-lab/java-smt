...
public class SolverOverviewServlet extends HttpServlet {
  ...
  public void printSolverInformation(PrintWriter out) throws InterruptedException, SolverException {
      final List<SolverOverviewServlet.SolverInfo> infos = new ArrayList<>();
      for (Solvers s : Solvers.values()) {
        infos.add(new SolverInfoPrinter().getSolverInformation(s));
      }

      infos.sort(Comparator.comparing(SolverOverviewServlet.SolverInfo::getName)); // alphabetical ordering

      SolverOverviewServlet.RowBuilder rowBuilder = new SolverOverviewServlet.RowBuilder();
      for (SolverOverviewServlet.SolverInfo info : infos) {
  ...
  /*
   *  This is more or less a copy of the JavaSMT example located at:
   *  java-smt/src/org/sosy_lab/java_smt/example/SolverOverviewTable.java
   */ 
