...
# #################################################################################
#
# Easier way of building: Download Yices2 dll for Windows from the website.
# Build GMP on Linux with mingw for Windows.
# Use this script with both.
#
# Command to build GMP on Linux with mingw for Windows:
# ./configure --disable-static --enable-shared --host=x86_64-pc-mingw32 --build=x86_64-linux-gnu CC=x86_64-w64-mingw32-gcc CC_FOR_BUILD=gcc CPPFLAGS=-D__USE_MINGW_ANSI_STDIO LDFLAGS="-static-libgcc -static-libstdc++"
# 
#
#
#
...
# check requirements
if [ ! -f "$YICES_SRC_DIR/bin/libyices.dll" ]; then
    echo "You need to specify the directory with the built yices2 on the command line!"
    echo "Can not find $YICES_SRC_DIR/bin/libyices.dll"
    exit 1
fi
...
x86_64-w64-mingw32-gcc -g -o $OUT_FILE -shared -Wl,-soname,$OUT_FILE \
    ...
    -I$YICES_SRC_DIR/include -L$YICES_SRC_DIR/lib -I$SHARED_GMP_SRC_DIR/include -L. \
    org_sosy_1lab_java_1smt_solvers_yices2_Yices2NativeApi.c \
    -lyices $YICES_SRC_DIR/bin/libyices.dll -lgmp -L$SHARED_GMP_SRC_DIR/lib \
    -lstdc++
